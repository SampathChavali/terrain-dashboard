...
export function Sidebar({ currentView, onNavigate, onHelp, onLogout, stats }: SidebarProps) {
  ...
  return (
    <aside className="w-[220px] shrink-0 flex flex-col glass-sidebar h-full overflow-hidden">
      <div className="p-5 pb-3">
        <div className="flex items-center gap-2 mb-5">
          <div className="w-8 h-8 rounded-full bg-green-primary flex items-center justify-center">
            <span className="text-white text-xs font-bold">T</span>
          </div>
          <span className="text-lg font-bold text-white">Terrain</span>
        </div>
        <p className="text-[10px] font-semibold text-white/60 uppercase tracking-widest mb-3">Menu</p>
        <nav className="space-y-1">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                {active && (
                  <span className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-green-light rounded-r-full -ml-3" />
                )}
                <span className={active ? 'text-green-light' : ''}>{item.icon}</span>
      ...
      <div className="px-5 mt-auto pb-5">
        <p className="text-[10px] font-semibold text-white/60 uppercase tracking-widest mb-2">General</p>
        <nav className="space-y-1">
          <button className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-white/80 hover:bg-white/10 hover:text-white cursor-pointer">
            <Settings className="w-4 h-4" />
            Settings
          </button>
          <button
            onClick={onHelp}
            className="w-full flex items-center gap-3 px-3 py-2 rounded-xl text-sm text-white/80 hover:bg-white/10 hover:text-white cursor-pointer"
          >
